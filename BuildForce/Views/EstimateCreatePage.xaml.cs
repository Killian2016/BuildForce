...
public partial class EstimateCreatePage : ContentPage
{
    ...
    public EstimateCreatePage(ApiService api)
    {
        InitializeComponent();
        _api = api;
        EstimateDatePicker.Date = DateTime.Today;
        ValidUntilPicker.Date = DateTime.Today.AddDays(30);
        AddLineItemRow();
        LoadProjects();
    }
    ...
    private void AddLineItemRow()
    {
        ...
        {
            Description = MakeEntry("Description (e.g. Kitchen remodel labor)"),
    ...
    private async Task RemoveRowAsync(LineItemRow row)
    {
        if (_rows.Count <= 1)
        {
            await DisplayAlert("Line Items", "An estimate needs at least one line item.", "OK");
            return;
        }
        _rows.Remove(row);
    ...
    private async void OnSave(object sender, TappedEventArgs e)
    {
        ...
        try
        {
            ...
            var result = await _api.CreateEstimateAsync(
                ...
                EstimateDatePicker.Date,
                ValidUntilPicker.Date,
                discountPct,
                NotesEditor.Text,
                items);
