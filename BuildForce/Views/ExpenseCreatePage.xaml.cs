...
public partial class ExpenseCreatePage : ContentPage
{
    ...
    // [PSH1] Dark bottom-sheet picker instead of the native Picker dialog.
    private void UpdateCategoryLabel()
    {
        bool has = !string.IsNullOrWhiteSpace(_selectedCategory);
        CategoryValueLabel.Text = has ? _selectedCategory : "\u2014 Select Category \u2014";
        CategoryValueLabel.TextColor = Color.FromArgb(has ? "#1a2340" : "#9aa3b8");
    }

    private async void OnPickCategory(object sender, TappedEventArgs e)
    {
        if (_categories.Count == 0) return;
        var sheet = new PickerSheetPage("Category", _categories, _selectedCategory);
        await Navigation.PushModalAsync(sheet);
        var picked = await sheet.Result.Task;
        if (picked != null)
        {
    ...
    private async void OnScanReceipt(object sender, TappedEventArgs e)
    {
        ...
        try
        {
            // [PSH3c] was DisplayActionSheet - no native chrome anywhere in this app.
            var scanSheet = new PickerSheetPage("Scan receipt", new List<string> { "Take Photo", "Choose from Gallery" });
            await Navigation.PushModalAsync(scanSheet);
            string action = await scanSheet.Result.Task ?? "";
            ...
            if (!string.IsNullOrWhiteSpace(result.SuggestedCategory))
            {
                ...
                // Keep the AI's suggestion even if it is not in the saved list,
                // instead of silently dropping it the way the index lookup did.
                if (match == null)
                {
                    _categories.Add(result.SuggestedCategory);
                    match = result.SuggestedCategory;
                }
    ...
    private async void OnSave(object sender, TappedEventArgs e)
    {
        ...
        var category = string.IsNullOrWhiteSpace(_selectedCategory) ? "General" : _selectedCategory;
