...
public partial class InvoiceDetailPage : ContentPage
{
    ...
    public InvoiceDetailPage(ApiService api, int id)
    {
        InitializeComponent();
        _api = api;
        _id = id;
    ...
    private async void LoadDetail()
    {
        try
        {
            var d = await _api.GetInvoiceDetailAsync(_id);
            if (d == null)
            {
                TitleLabel.Text = "Could not load invoice";
                await DisplayAlert("Error", "Could not load invoice details.", "OK");
                return;
            }

            TitleLabel.Text = d.InvoiceNumber;
            StatusLabel.Text = d.Status;
            MetaLabel.Text = d.PaidDate.HasValue ? $"Paid {d.PaidDate:MMM d, yyyy}" : "";

            string pillText, pillBg;
            switch (d.Status)
            {
                case "Paid":    pillText = "#0d7a4f"; pillBg = "#d8f5e8"; break;
                case "Overdue": pillText = "#b3261e"; pillBg = "#fde3e1"; break;
                case "Draft":   pillText = "#5b6472"; pillBg = "#e8ecf3"; break;
                default:        pillText = "#8a6100"; pillBg = "#fdf0d2"; break;
            }
            ...
            DateLabel.Text = d.InvoiceDate.ToString("MMM d, yyyy");
            DueLabel.Text = d.DueDate?.ToString("MMM d, yyyy") ?? "\u2014";
    ...
    private async void OnEmail(object sender, TappedEventArgs e)
    {
        ...
        try
        {
            var target = await DisplayPromptAsync("Email Invoice",
                "Send to:", "Send", "Cancel",
            ...
            var msg = await _api.EmailInvoiceAsync(_id, target.Trim());
            _delivering = false;
    ...
    private async void OnDownloadPdf(object sender, TappedEventArgs e)
    {
        ...
        try
        {
            ...
            var bytes = await _api.GetInvoicePdfAsync(_id);
            _delivering = false;
            ...
            var fileName = $"Invoice_{_docNumber ?? _id.ToString()}.pdf";
